...
export default {
  ...
  computed: {
    ...
    noDatasource () {
      const dsData = this.$store.getters.getDataSourceData
      if (dsData) {
        return false
      }
      return true
  ...
  methods: {
    ...
    async onOKSelectionClick () {
      ...
      const logO = this.logOperators
      const fil = this.filterselect
      ...
      const userSelectedValue = this.$store.getters.getUserSearchString
    ...
    async onLogOperatorButtonClick (value) {
      ...
      await this.$store.dispatch('passNewUserSelectedDescriptorState', null)
      const userSelectedDescripors = this.$store.getters.getUserSearchString // this.selectedItems
    ...
    onOKExportButtonClick (value) {
      ...
      if (selectedItems && selectedItems.length > 0) {
        ...
        const searchstring = { text: '', type: process.env.VUE_APP_SIMPLESEARCH_EXPORT_FILETYPE, version: this.appVersion, db_id: null, db_version: null, license: licenseText, licenseURI: licenseURI, items: null }
    ...
    onAddToListOfGroups () {
      ...
      if (selectedItems && selectedItems.length === 1) {
        ...
        const groupItem = { groupKey: searchKey, operator: currentOperator, groupValues: groupValueArray, text: newSearchStringPart, type: this.searchStringGroupItemTypeValues }
          ...
          for (let i = 0; i < selectedItems.length; i++) {
            ...
            if (!tempArray.includes(selectedItems[i].CID)) {
              ...
              for (let d = 0; d < filteredDescs.length; d++) {
                ...
                const groupItem = { groupKey: searchKey, groupValues: currentSelected }
      ...
      fileReader.addEventListener('load', e => {
        try {
          ...
          if (tempSearchCriteria && tempSearchCriteria.type && tempSearchCriteria.type === process.env.VUE_APP_SIMPLESEARCH_EXPORT_FILETYPE) {
            ...
            const tempCriteria = tempSearchCriteria.items
            ...
            if (tempCriteria.type === this.searchStringGroupItemTypeGroupItems) {
              for (const rec of tempCriteria.groupValues) {
